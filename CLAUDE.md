...
### Angular Material dialogs don't inherit the admin page's dark background
Admin page components set `background: #0f172a` via `:host`, but `MatDialog` panels render in a CDK overlay portal appended to `<body>` — outside the component's DOM subtree. Component-scoped CSS (including `::ng-deep`) cannot reach the overlay. On the global `theme-type: light` Material theme the dialog surface stays white, creating a jarring contrast against the dark admin UI. Fix: pass `panelClass: 'dark-dialog'` in the `dialog.open()` options and add `.dark-dialog { --mdc-dialog-container-color: #1e293b; } .dark-dialog .mdc-dialog__surface { border: 1px solid #334155 !important; border-radius: 12px !important; }` to `styles.scss` — global styles are the only CSS that can reach CDK overlay portals.
