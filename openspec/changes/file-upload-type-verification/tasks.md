...
- [ ] 1.0 CONTRACT — write openspec/changes/file-upload-type-verification/contracts/group-1.md with the ### Contract block above; confirm all three fields (Spec, Runtime, Code) are non-empty before proceeding
- [ ] 1.1 Add `tika-core` and `tika-parser-microsoft-module` (version 3.1.0) to `backend/pom.xml`
- [ ] 1.2 Rename `StorageProperties.blockedExtensions` → `allowedExtensions`; update `application.yml` `blocked-extensions` → `allowed-extensions` with default `pdf,jpg,jpeg,png,xlsx,xls,csv,doc,docx`; update env var reference from `BLOCKED_EXTENSIONS` to `ALLOWED_EXTENSIONS`
- [ ] 1.3 RED — write `StoragePropertiesTest` verifying that `allowedExtensions` binds correctly from `application.yml` and that `blockedExtensions` no longer exists on the record
- [ ] 1.4 GREEN — confirm test passes (property binding is done in 1.2; fix any binding issues)
...
- [ ] 2.0 CONTRACT — write openspec/changes/file-upload-type-verification/contracts/group-2.md with the ### Contract block above
- [ ] 2.1 RED — write `FileUploadValidatorTest` cases: disallowed extension (`.zip`) rejected with 400; no extension rejected; allowed extension (`.pdf`) accepted; existing size/filename tests still pass
- [ ] 2.2 GREEN — update `FileUploadValidator.validate()` signature to `(MultipartFile file)`; rewrite `validateExtension()` to use allowlist; extract filename/size from `MultipartFile`
- [ ] 2.3 Update callers: `MeDocumentService.uploadMyDocument()` and `DocumentService.upload()` — pass `file` directly to `fileUploadValidator.validate(file)`
- [ ] 2.4 RED — add integration-level tests in `DocumentServiceTest` confirming disallowed extension throws `FileValidationException`
- [ ] 2.5 GREEN — confirm all tests pass after caller updates
...
- [ ] 3.0 CONTRACT — write openspec/changes/file-upload-type-verification/contracts/group-3.md with the ### Contract block above
- [ ] 3.1 RED — write `FileUploadValidatorTest` cases for Layer 2: `Content-Type: application/octet-stream` with `.pdf` extension rejected; `Content-Type: application/zip` with `.xlsx` extension accepted (Layer 2 permissive)
- [ ] 3.2 GREEN — add `validateDeclaredMimeType(MultipartFile file)` using hardcoded extension → MIME types map; wire into `validate()` after `validateExtension()`
- [ ] 3.3 RED — write `FileUploadValidatorTest` cases for Layer 3: actual `.exe` bytes named `evil.pdf` rejected; actual `.zip` bytes named `evil.xlsx` rejected; actual `.pdf` bytes accepted; `.csv` text file accepted; `.xlsx` OOXML file accepted
- [ ] 3.4 GREEN — add `validateActualMimeType(MultipartFile file)` using `Tika.detect()`; wire into `validate()` after `validateDeclaredMimeType()`
...
- [ ] 4.1 Run full backend test suite: `cd backend && ./mvnw test` — ensure no regressions
- [ ] 4.2 Run full frontend test suite: `cd frontend && npx ng test --no-watch` — ensure no regressions
- [ ] 4.3 Run e2e suite if servers are up: `cd e2e && npx playwright test` — ensure upload happy path still passes
- [ ] 4.4 Run superpowers:verification-before-completion
