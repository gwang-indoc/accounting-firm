...
- **THEN** all four user-visible form fields MUST be cleared.
...
On every submission result, the application SHALL surface feedback to the visitor through a `MatSnackBar` notification. No inline alert banner is used.

#### Scenario: Success snackbar appears after 202

- **WHEN** the backend returns HTTP 202
- **THEN** a `MatSnackBar` MUST appear with the text "Thanks — we'll reply soon" AND MUST auto-dismiss after approximately 3 seconds.

#### Scenario: Error snackbar appears after failure

- **WHEN** the backend returns HTTP 4xx or 5xx (other than honeypot silent-accept)
- **THEN** a `MatSnackBar` MUST appear with an error message AND an "OK" action button to dismiss.
