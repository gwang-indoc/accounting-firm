# admin-client-export Specification

## Purpose
Admins had no way to bulk-export client data — only one-at-a-time document downloads existed. This capability adds a self-service export flow: admins multi-select up to 200 clients, configure what to include (metadata CSV, documents ZIP, or combined), and trigger a streaming download. The server enforces ownership so only the authenticated admin's own clients are exportable.
## Requirements
