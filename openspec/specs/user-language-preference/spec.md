# user-language-preference Specification

## Purpose
Persist each authenticated user's language preference (EN or ZH) in the database so their locale is consistent across devices and sessions. On login, the profile language is applied immediately; on language switch, the preference is saved to the profile in the background.
## Requirements
