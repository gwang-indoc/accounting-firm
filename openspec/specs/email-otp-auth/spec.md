# email-otp-auth Specification

## Purpose
Provides passwordless email-code (OTP) authentication for clients. A user requests a 6-digit code by email; entering the code logs them in or, for a first-time email, starts a one-step account creation flow. Replaces email/password login and registration.
## Requirements
