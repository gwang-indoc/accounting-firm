...
**Frontend**: Checkbox column in admin client list; "Select all" fetches `GET /api/clients/ids`; `effect()` clears selection on filter change; export toolbar with count badge, "Clear" deselect-all, and "Export" button; `AdminExportDialogComponent` (metadata/documents checkboxes + year selector); `AdminExportService` blob POST → Content-Disposition filename → `URL.createObjectURL` anchor download.
