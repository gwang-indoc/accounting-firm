...
**Backend**: `clients.email` NOT NULL + UNIQUE; `clients.admin_id` NOT NULL FK to `users`; CRUD queries filtered by `admin_id`; `POST /api/clients` validates email against users table (400) and duplicate guard (409). `business_type` VARCHAR(20) NOT NULL (PERSONAL/CORPORATE/SELF_EMPLOYED); `fiscal_year_end_month` / `fiscal_year_end_day` SMALLINT NOT NULL (V13 migration); PERSONAL always stores 12/31; CORPORATE/SELF_EMPLOYED require valid `MonthDay`. `ClientDto` includes `activeEngagementStatus` (nullable) — computed via two Spring Data derived queries: latest non-COMPLETED engagement by tax year, falling back to latest overall; null if no engagements.
**Frontend**: Add Client dialog — email field triggers debounced lookup, auto-fills name on match, blocks submission for unregistered/duplicate email; business type select; conditional FYE month/day inputs (hidden for PERSONAL). Client list — Workflow State column shows `activeEngagementStatus` (or "—"); Workflow State dropdown filter (`''` = All, status values, `'__none__'` = no engagements); Workflow action button per row navigates to `/admin/clients/:id/workflow`.
